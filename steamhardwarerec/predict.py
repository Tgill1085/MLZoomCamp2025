...
def load_data():
    ...
    if not os.path.exists(csv_path):
        ...
        st.info("""
        This file is generated by the data gathering notebook.
        
        **To fix:**
        1. Run `data_gathering.ipynb` locally
        2. Commit and push the generated `steam_games_final.csv`
        3. Redeploy the app
        """)
        st.stop()
    ...
    if model is None:
        st.warning("No valid model found — training a new one (this takes 1-2 minutes)...")
        with st.spinner("Training model..."):
            result = subprocess.run(["python", "train.py"], capture_output=True, text=True)
            if result.returncode == 0:
                st.code("Training successful!")
                if os.path.exists(model_path):
                    model = joblib.load(model_path)
                    st.success("Fresh model trained and loaded!")
                else:
                    st.error("Training completed but model file not created.")
                    st.stop()
            else:
    ...
    if not os.path.exists(cpu_csv) or not os.path.exists(gpu_csv):
        st.error("Benchmark files missing. Run data gathering.")
        st.stop()
