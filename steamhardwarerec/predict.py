...
# === Fix working directory & path handling ===
script_dir = os.path.dirname(os.path.abspath(__file__))
os.chdir(script_dir)  # Ensure we're in the correct folder
...
def load_data():
    ...
    if not os.path.exists(csv_path):
        st.error(f"Data file 'steam_games_final.csv' not found in {script_dir}!")
        st.info("""
        This file should be generated by running `data_gathering.ipynb`.
        
        **To fix:**
        1. Run the data gathering notebook locally
        2. Commit and push `steam_games_final.csv` to the repo
        3. Redeploy the app
        
        Alternatively, remove this check if you want auto-generation.
        """)
        st.stop()
    
    df = pd.read_csv(csv_path)
    
    # Model loading with auto-training fallback
    ...
    if model is None:
        st.warning("No pre-trained model found — training a new one (this may take 1-2 minutes)...")
        with st.spinner("Training model..."):
            ...
            if result.returncode == 0:
                ...
                if os.path.exists(model_path):
                    model = joblib.load(model_path)
                    st.success("Fresh model trained and loaded!")
                else:
                    st.error("Training completed but model file not found.")
                    st.stop()
            else:
    ...
    if not os.path.exists(cpu_csv) or not os.path.exists(gpu_csv):
        st.error("Benchmark CSV files missing. Run data gathering notebook.")
        st.stop()
